...
export class AgentSession {
  ...
  async generateWriterTurn(step: WorkflowStep, previousCriticFeedback?: string): Promise<string> {
    try {
      ...
      // Use Gemini 1.5 Flash (Stable, High RPM)
      return await this.callGeminiAPI('gemini-1.5-flash', prompt, finalKey);
  ...
  async generateCriticTurn(step: WorkflowStep, writerDraft: string): Promise<string> {
    ...
    if (geminiKey) {
      ...
      try {
        ...
        // Use Gemini 1.5 Flash (Stable, High RPM)
        return await this.callGeminiAPI('gemini-1.5-flash', prompt, geminiKey);
